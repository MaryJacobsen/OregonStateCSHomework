...
int main(int argc, char* argv[])
{
	...
	char* DecryptedMsg = malloc(info->FileLength * sizeof(char*));
	char serverAccept;
	...
	//send D so server knows which client this is
	...
	{
		close(socketFD);
		error("Error: not the decryption server");
	}
	...
	recv(socketFD, DecryptedMsg, info->FileLength * sizeof(char), 0);
	printf("%s\n", DecryptedMsg);
