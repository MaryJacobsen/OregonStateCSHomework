...
char* EncryptMsg(char* key, char* message)
{
	int i; 
	int keytemp; 
	int msgtemp;
	...
	for(i=0; i<length; i++){
		...
		else
		{
			...
			//add message and key values together and mod
			encryptedMsg[i] = (char)(msgtemp + (keytemp % 3));
		}
	}
	...
	while(1)
	{
		...
		switch(pid)
		{
			...
			case 0:
				...
				//encrypt message
				encryptedMsg = EncryptMsg(KeyBufferFile, MsgBufferFile);

				//send encrypted message back to client and shut down connection
				send(establishedConnectionFD, encryptedMsg, FileLength * sizeof(char), 0);
