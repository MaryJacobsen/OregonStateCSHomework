...
void adjustHeap(DynamicArray* heap, int last, int position, compareFunction compare)
{
	...
	if(rightChild < last) /*We have two children*/
	{
		...
		if(compare(dyGet(heap, position), dyGet(heap, smallestChild)) == 1) //if value at pos > value of smallest child
		{
			dySwap(heap, position, smallestChild); //swap the two values
		    adjustHeap(heap, last, smallestChild, compare); //adjust the heap
		}
	}
	else if(leftChild < last) //if we have 1 child
	{
		if(compare(dyGet(heap, position), dyGet(heap, leftChild)) == 1) //if value at pos > value of child
		{
			dySwap(heap, position, leftChild); //swap the two values
			adjustHeap(heap, last, leftChild, compare); //adjust the heap
		}
	}
		//if no children, no action required
}
...
{
    // FIXME: implement
	dyAdd(heap, value); //add newValue to end of heap
	int pos = dySize(heap) - 1;
	int parent;
	while(pos != 0)
	{
		parent = (pos-1)/2; //set parent index
	    if(compare(dyGet(heap, pos), dyGet(heap, parent)) == -1) //if value at pos < value at parent
	    {
	    	dySwap(heap, parent, pos); //swap the two values
	        pos = parent; //set pos for next iteration (percolate up)
	    }
	    else
	    	return;
	    }
