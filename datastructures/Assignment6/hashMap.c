...
int* hashMapGet(HashMap* map, const char* key)
{
	...
	int index = (int) (labs(hash) % map->capacity);
...
void resizeTable(HashMap* map, int capacity)
{
	...
	for(i = 0; i < oldtc; i++)
	{
	 ...
	 free(oldTable); /* Free up the table */
}
...
void hashMapPut(HashMap* map, const char* key, int value)
{
	...
	map->size++;
	if ((map->size / (double) map->capacity) > 8.0) resizeTable(map, 2*(map->capacity));
}
